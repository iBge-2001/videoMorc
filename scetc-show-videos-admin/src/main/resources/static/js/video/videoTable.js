...
$("#toolbar .btn-group .btn").click(function() {
 ...
 var selectVideoType = function() {
 var params = {
 "data" : "all"
 };
 $.ajax({
 url : '../../video/selectVideoType',
 type : 'post',
 data : params,
 dataType : 'json',
 success : function(data) {
	 ...
	 var typeName = '';
		...
		for (var i = 0; i < rows.length; i++) {
		    // if(rows[i].videoCategory == 'define')
		    // {
			// 	typeName += '<li><a href="javascript:void(0);" style="font-weight: bold;font-size: 13px;color: black;" onclick="sendType(\'' + rows[i].id + '\')">美拍类</a></li>';
		    // }
		    // else if(rows[i].videoCategory == 'food')
		    // {
			// 	typeName += '<li><a href="javascript:void(0);" style="font-weight: bold;font-size: 13px;color: black;" onclick="sendType(\'' + rows[i].id + '\')">美食类</a></li>';
		    // }
		    // else if(rows[i].videoCategory == 'beauty')
		    // {
			// 	typeName += '<li><a href="javascript:void(0);" style="font-weight: bold;font-size: 13px;color: black;" onclick="sendType(\'' + rows[i].id + '\')">美妆类</a></li>';
		    // }
			typeName = '<li><a href="javascript:void(0);" style="font-weight: bold;font-size: 13px;color: black;" onclick="sendType( \''+rows[i].name+'\'  )">'+rows[i].name +'</a></li>';
			// <li><a href="javascript:void(0);" style="font-weight: bold;font-size: 13px;color: black;"onclick="selectByType( \''+rows[i].name+'\'  )" >'+rows[i].name +'</a></li>;
		}
		// 初始化数据
...
// 获取行号
 ...
 // var selectByType = function (type_name) {
	//  var params = $('#allBlog').bootstrapTable('getOptions')
	// 	queryParams = {
	// 		 pageSize: params.limit,
	// 		 page: (params.offset) / params.limit + 1,
 ...
 // }
// 传参数类别ID
var sendType = function(type_name) {
	var params = $('#allBlog').bootstrapTable('getOptions')
	params.queryParams = function(params) {
		return {
			pageSize : params.limit,
	...
	$('#allBlog').bootstrapTable({
		...
		columns : [
			...
			},
			{
				...
				formatter : function(value, row, index) {
					var index1 = index + 1;
					var id = '<span title="ID:' + row.id + '">' + row.nickName + '</span>';
					return id;
				}
			...
			
			{
				...
				formatter : function(value, row, index) {
					var index1 = index + 1;
					var id = '<span title="ID:' + row.id + '">' + row.likeCounts + '</span>';
					return id;
				}
			...
			},
			{
				...
				formatter : function(value, row, index) {
					var index1 = index + 1;
					var id = '<span title="ID:' + row.id + '">' + row.videoSeconds + '</span>';
					return id;
				}
			...
			},
			{
				...
				formatter : function(value, row, index) {
					...
					// 删除
					var b  = '<a  class=" btn-sm btn-danger"  onclick="operation(\'' + row.id + '\',1)"><i class="fa fa-share-square-o" ></i>删除</a> ';
					// 从回收站还原
					var c  = '<a  class=" btn-sm btn-danger"  onclick="operation(\'' + row.id + '\',2)"><i class="fa fa-share-square-o" ></i>修改</a> ';
					// 从回收站还原
			    return a+b+c;
			}
			}

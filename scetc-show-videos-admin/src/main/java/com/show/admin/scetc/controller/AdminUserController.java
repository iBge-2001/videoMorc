...
public class AdminUserController extends BasicController {
    ...
    @SysLog
    @PostMapping("/logout")
    public LybJsonResult logout(HttpServletRequest request, HttpServletResponse response) {
    ...
    public LybJsonResult changePasswordSubmit(HttpServletRequest request, HttpServletResponse response,
                                              String oldPassword, String newPassword, String reNewPassword) {

        AdminUser adminUser = (AdminUser) request.getSession().getAttribute("adminUser");
        if (adminUser == null) {
            return LybJsonResult.errorMsg("修改账号密码失败,请重新登陆");
        }
        if (!reNewPassword.equals(newPassword)) {
            return LybJsonResult.errorMap("两次输入密码不一致");
        }
        adminUser = adminUserService.selectOneById(adminUser.getId());
        if (!adminUserService.check(oldPassword, adminUser)) {
            return LybJsonResult.errorMsg("修改密码失败,初始密码错误");
        }
    ...
    @SysLog
    public LybJsonResult loginSubmit(HttpServletRequest request, String username, String password, String verifyCode,
                                     Model model) {
        if (!ImageCodeUtils.checkImageCode(request.getSession(), verifyCode)) {
            return LybJsonResult.errorMsg("验证码错误");
        ...
        if (adminUser == null) {
            // 登陆失败
            return LybJsonResult.errorMsg("账号密码错误");
        }
        // 检查账号是否被禁用了
    ...
    @SysLog
    public LybJsonResult queryAllAdminUser() {
        return LybJsonResult.ok(adminUserService.queryAll());
    }

...
public class UserServiceImpl implements UserService {
	...
	@Override
	public List<Users> getFollowings(String fanId) {
		List<Users> followingsList = new ArrayList<>();

		List<UsersFans> userFansList = userFansMapper.findByUserId(fanId);
		for (UsersFans userFans : userFansList) {
			Users user = userMapper.findById(userFans.getUserId());
			System.out.println(user);
			if (user != null) {
				Users userInfo = new Users();
				userInfo.setFaceImage(user.getFaceImage());
				userInfo.setNickname(user.getNickname());
				userInfo.setFansCounts(user.getFansCounts());
				userInfo.setFollowCounts(user.getFollowCounts());

				followingsList.add(userInfo);
			}
		}

...
public class RegistLoginController extends BasicController {
	...
	@PostMapping("/login")
	public LybJsonResult login(@RequestBody Users user) throws Exception {
		Thread.sleep(500);
		if (StringUtils.isBlank(user.getUsername()) || StringUtils.isBlank(user.getPassword())) {
			return LybJsonResult.errorMsg("小主,账号密码不能为空哦");
		}
...
//				redis.set(userVo.getUserToken(), userVo.toString());
				return LybJsonResult.ok(userVo);

			} else {
				return LybJsonResult.errorMsg("小主,你的账号密码错误");
			}
		} else {
			return LybJsonResult.errorMsg("小主,你的账号密码错误");
		}
	...
	@PostMapping("/logout")
	public LybJsonResult logout(String userId) throws InterruptedException {
		//
	...
	@ApiImplicitParam(name = "userId", value = "用户id", required = true, dataType = "String", paramType = "query")
	@PostMapping("/regist")
	public LybJsonResult regist(@RequestBody Users user) {
		// 1.用户和密码是否为空 不能为空
		if (StringUtils.isBlank(user.getUsername()) || StringUtils.isBlank(user.getPassword())) {
			return LybJsonResult.errorMsg("小主,用户名和密码不能为空哦");
		}
		...
		if (!usernameIsExist) {
			...
			return LybJsonResult.errorMsg("用户已经存在,换一下吧 (●'◡'●)");
		}
		user.setPassword("");
...
//		redis.set(User_REDIS_SESSION + ":" + user.getId(), token, 60 * 1000 * 30);
	...
	public UsersVo setUserRedisSessionToken(Users userModel) {
		String token = UUID.randomUUID().toString();
		redis.set(User_REDIS_SESSION + ":" + userModel.getId(), userModel.toString(), 60 * 1000 * 30);
		UsersVo userVo = new UsersVo();
